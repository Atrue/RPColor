...
exports.validNick = function(nickname) {
    var regex = /.*/;
    return regex.exec(nickname) !== null && regex.exec(nickname)[0].trim() !== "" ;
};
...
exports.massToEvolution = function(mass){
    if (mass < 30){
        return parseFloat(mass-10)/20 * 0.25; //0.25
    }
    if (mass < 70){
        return parseFloat(mass - 30)/160 + 0.25;
    }
    if (mass < 150){
        return parseFloat(mass - 70)/320 + 0.5;
    }
    if (mass < 400){
        return parseFloat(mass - 150)/1000 + 0.75;
    }
    return 1;
};
